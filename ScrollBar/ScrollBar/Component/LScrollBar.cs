...
namespace SrollBarControl
{
    ...
    public class LScrollBar:Control
    {
        ...
        /// 背景条颜色
        /// </summary>
        [Category("ScrollBar"), Description("背景条颜色")]
        ...
        /// 滑块颜色
        /// </summary>
        [Category("ScrollBar"), Description("滑块颜色")]
        ...
        /// </summary>
        [Category("ScrollBar"), Description("鼠标进入时滑块颜色\r\n默认：绿")]
        ...
        /// </summary>
        [Category("ScrollBar"), Description("是否是圆角\r\n默认：是")]
        ...
        /// </summary>
        [Category("ScrollBar"), Description("方向\r\n默认：水平，从左到右")]
        ...
        /// 滑条高度（水平）/宽度（垂直）
        /// </summary>
        [Category("ScrollBar"), Description("滑条高度（水平）/宽度（垂直")]
        ...
        /// 滑块位置
        /// </summary>
        [Category("ScrollBar"), Description("滑块位置"),Browsable(false)]
        public float L_Position { get; private set; } = 0f;

        private float _CtrlShowLen=1f;
        /// <summary>
        /// 显示长度/每页长度
        /// </summary>
        [Category("ScrollBar"), Description("显示长度/每页长度")]
        public float L_CtrlShowLen
        {
            get { return _CtrlShowLen; }
            set
            {
                _CtrlShowLen = value;
                pInit();
        ...
        private float _CtrlLen=1f;
        /// <summary>
        /// 文档长度
        /// </summary>
        [Category("ScrollBar"), Description("文档长度")]
        public float L_CtrlLen
        {
            get { return _CtrlLen; }
            set
            {
        ...
        /// 滑块可移动距离
        /// </summary>
        [Category("ScrollBar"), Description("滑块可移动距离"), Browsable(false)]
        public float L_SliderMoveRange { get; private set; }
        /// <summary>
        /// 滑块长度
        /// </summary>
        [Category("ScrollBar"), Description("滑块长度"), Browsable(false)]
        public float L_SliderLength { get; private set; }
        private float _ShowPosition = 0f;
        /// <summary>
        /// 显示位置实际数值（正数）
        /// </summary>
        [Category("ScrollBar"), Description("显示位置（正数）"),Browsable(false)]
        ...
        private float _ShowPositionPercent = 0f;
        /// <summary>
        /// 显示位置的百分比数值（%）
        /// </summary>
        [Category("ScrollBar"), Description("显示位置的百分比数值（%）"), Browsable(false)]
        public float L_ShowPositionPercent
        {
            get { 
                Console.WriteLine("_ShowPosition "+ _ShowPosition + " L_CtrlLen   "+ L_CtrlLen);
                return _ShowPosition / (L_CtrlLen - L_ScrollInterval); }
            set
            {
                _ShowPositionPercent = value;
                pShowPositionToPosition();
                Invalidate();
            }
        }
        ...
        /// </summary>
        [Category("ScrollBar"), Description("滑块最小长度\r\n默认：20")]
        public float L_SliderMiniSize { get; set; } = 20f;
        #endregion

        #region 

        private float fCtrlShowRatio, fShowScrollRatio, fAbove, fBelow, fCapWidth, fCapHalfWidth;
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (i != -1)
            {
                if (i == 0)
                {
                    //点在了滑块上方空白位置
                    //‘上一页’，其实是减去_Page的量
                    _ShowPosition -= _CtrlShowLen;
                    if (_ShowPosition < 0) _ShowPosition = 0;
                    pShowPositionToPosition();
                    Invalidate();
                    LScrolled?.Invoke(this, new LScrollEventArgs(L_Position, L_ShowPositionPercent));
                }
                else if (i == 1)
                {
                    //点在了滑块上
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (mouseStatus == MouseStatus.Down)
            {
                ...
                Invalidate();
                Console.WriteLine("L_ShowPositionPercent   " + L_ShowPositionPercent);
                LScrolled?.Invoke(this, new LScrollEventArgs(L_Position, L_ShowPositionPercent));
            }
        }
